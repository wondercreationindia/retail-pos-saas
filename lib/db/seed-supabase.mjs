...
if (parseInt(existingOrders.rows[0].count) === 0) {
  await pool.query(`
    ...
    VALUES
      ($1, $2, 'ORD-0001', 'completed', '[{"name":"Espresso","qty":2,"price":3.50}]', 7.00, 0.56, 7.56, 'cash'),
      ($1, $2, 'ORD-0002', 'completed', '[{"name":"Cappuccino","qty":1,"price":4.50},{"name":"Granola Bar","qty":2,"price":2.49}]', 9.48, 0.76, 10.24, 'card'),
      ($1, $2, 'ORD-0003', 'pending',   '[{"name":"USB Cable","qty":1,"price":9.99}]', 9.99, 0.80, 10.79, 'cash'),
      ($1, $2, 'ORD-0004', 'completed', '[{"name":"Green Tea","qty":3,"price":2.75}]', 8.25, 0.66, 8.91, 'card')
  `, [tenantId, adminId]);
}
